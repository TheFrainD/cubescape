...
void buffer_destroy(buffer_t *buffer) {
    ...
    if (buffer->id == 0) {
        LOG_WARN("'buffer_destroy' called with 0 buffer ID");
        return;
    }
...
void buffer_bind_range(const buffer_t *const buffer, const uint32_t binding_point, size_t size) {
    ...
    if (size > buffer->size) {
        LOG_WARN("'buffer_bind_range' called with invalid size");
        return;
    }
...
void buffer_data(const buffer_t *buffer, size_t size, const void *data) {
    if (buffer == NULL) {
        LOG_WARN("'buffer_data' called with NULL buffer");
        return;
    }
...
void buffer_sub_data(const buffer_t *buffer, size_t offset, size_t size, const void *data) {
    if (buffer == NULL) {
        LOG_WARN("'buffer_sub_data' called with NULL buffer");
        return;
    }

    if (offset + size > buffer->size) {
        LOG_WARN("'buffer_sub_data' called with invalid offset and size");
        return;
    }
